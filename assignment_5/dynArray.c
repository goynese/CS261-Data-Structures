...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
	...
	if(compare(heap->data[j], heap->data[i]) == 1)
		return i;
	if(compare(heap->data[j], heap->data[i]) == -1)
		return j;
	return i;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if(rghtidx < max)
	{
		minidx = _smallerIndexHeap(heap, leftidx, rghtidx);
		if((_smallerIndexHeap(heap, minidx, pos)) == minidx)
	...
	{
		if(_smallerIndexHeap(heap, pos, leftidx) == leftidx)
...
{
	int notheap = (heap->capacity/2)-1;
	assert(heap != NULL);
	if(heap->size > 0)
	{
		do{
			_adjustHeap(heap, heap->capacity, notheap);
			notheap--;
