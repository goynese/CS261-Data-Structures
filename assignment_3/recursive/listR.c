...
int listContains (struct list *lst, TYPE e) {
	...
	newlst->head = lst->head->next;

	if(EQ(newlst->head, newlst->tail))
		return 0;

	if(EQ(newlst->head->value, e))
		return 1;

	listContains( newlst , e);
...
void listRemove (struct list *lst, TYPE e) {
	...
	if(lst->size == 1)
	{
		_removeLink(newlst, newlst->head->next);
		lst--;
	}

	if(EQ(newlst->head->value, e))
	{
		_removeLink(newlst, newlst->head);
		return;
	}
